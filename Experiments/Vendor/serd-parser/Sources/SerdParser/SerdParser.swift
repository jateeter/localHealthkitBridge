...
// TODO: handle and propogate errors
...
public class SerdParser {
    ...
    let statement_sink : @convention(c) (UnsafeMutableRawPointer?, SerdStatementFlags, UnsafePointer<SerdNode>?, UnsafePointer<SerdNode>?, UnsafePointer<SerdNode>?, UnsafePointer<SerdNode>?, UnsafePointer<SerdNode>?, UnsafePointer<SerdNode>?) -> SerdStatus = { (handle, flags, graph, subject, predicate, object, datatype, language) -> SerdStatus in
        ...
        do {
            let s = try SerdParser.node_as_term(env: env, node: subject.pointee, datatype: nil, language: nil)
            let p = try SerdParser.node_as_term(env: env, node: predicate.pointee, datatype: nil, language: nil)
            let o = try SerdParser.node_as_term(env: env, node: object.pointee, datatype: datatype?.pointee.value, language: language?.pointee.value)
    ...
    private static func node_as_term(env: OpaquePointer?, node: SerdNode, datatype: String?, language: String?) throws -> RDFTerm {
        ...
        case SERD_CURIE:
            var n = node
            let t = withUnsafePointer(to: &n) { (n) -> RDFTerm? in
                ...
                return .iri("\(uri_prefix.value)\(suffix.value)")
            }
            
            guard let term = t else {
                throw SerdParserError.parseError("Undefined namespace prefix \(node.value)")
            }
            return term
        default:
    ...
    @discardableResult
    public func parse(string: String, handleTriple: @escaping (RDFTerm, RDFTerm, RDFTerm) -> Void) throws -> Int {
    ...
    public func parse(file filename: String, handleTriple: @escaping TripleHandler) throws -> Int {
        guard let input = serd_uri_to_path(filename) else { throw SerdParserError.parseError("no such file") }
        ...
        try withUnsafePointer(to: &context) { (ctx) throws -> Void in
            ...
            guard let in_fd = fopen(filename, "r") else {
                ...
                throw SerdParserError.parseError("\(error)")
            }
